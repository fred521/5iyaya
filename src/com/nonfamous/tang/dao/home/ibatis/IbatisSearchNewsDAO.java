 ...
 * 
 */
public class IbatisSearchNewsDAO extends SqlMapClientDaoSupport implements
		SearchNewsDAO {
	...
	private static final String QuartzName = "news";
	...
	public void createNewsIndex() {
		...
		if (!indexFile.exists() || !indexFile.canWrite()) {
			if (logger.isErrorEnabled())
				logger.error("资讯索引文件目录创建失败或不可写入！");
			return;
		}
	...
	public SearchNewsQuery findAllNews(SearchNewsQuery query) {
		File indexFile = new File(indexFilePath);
		if (!indexFile.exists())
			throw new ServiceException("资讯索引目录不存在");
		try {
	...
	private void indexUpdate(File indexFile, boolean isCreate, boolean isRebuild) {
		...
		Date updateDate = newsDAO.getSysDate();
		query.setEnd(updateDate);
		query.setCurrentPage(1);
		List<NewsBaseInfo> list = newsDAO.getNewsListForIndex(query);
		...
		synchronized (writelock) {
			try {
				...
				try {
					...
					query.setBegin(DateUtils.getDiffDateFromEnterDate(quartzLog
							.getGmtExecute(), -1000));
					list = newsDAO.getNewsListForIndex(query);
				} finally {
					if (reader != null) {
						try {
							reader.close();
				...
				while (query.getCurrentPage() < query.getTotalPage()) {
					...
					indexNews(writer, newsDAO.getNewsListForIndex(query),
							isCreate);
				}
